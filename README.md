...
- `plugin/apps/data-app/index.mjs`：**module 入口**（导出 `mount({ container, host, slots })`）
- `plugin/apps/data-app/compact.mjs`：**compact 入口**（可选；用于侧边抽屉/分栏场景）
- `plugin/backend/index.mjs`：**插件后端**（导出 `createUiAppsBackend(ctx)`，通过 `host.backend.invoke()` 调用）
- `plugin/apps/data-app/mcp-server.mjs`：应用自带 MCP Server（可选）
- `plugin/apps/data-app/mcp-prompt.zh.md` / `.en.md`：MCP Prompt（可选）
...
⚠️ ChatOS 导入插件时会排除 `node_modules/`。因此 MCP server/后端只要用了第三方依赖（如 `@modelcontextprotocol/sdk`、`zod`、`mongodb`），就必须先 bundle 成单文件，或把依赖源码放进插件目录。
若看到 `Cannot find package '@modelcontextprotocol/sdk'`，说明依赖未被 bundle。

如果你要启用 MCP：

1) 运行 `npm run bundle` 生成 bundle（会输出到 `plugin/backend/index.bundle.mjs` 与 `plugin/apps/data-app/mcp-server.bundle.mjs`）  
2) `plugin/plugin.json` 已指向 bundle 产物  
